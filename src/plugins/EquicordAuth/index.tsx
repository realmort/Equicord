...
export default definePlugin({
    ...
    showBlock() {
        ...
        hwidBox.textContent = "\u2605 \u2605 \u2605 \u2605 \u2605 \u2605 \u2605 \u2605";
        hwidBox.style.cssText = "color:#555;font-size:18px;letter-spacing:4px;background:#0a0a0a;padding:12px 20px;border-radius:8px;border:1px solid #222;width:100%;text-align:center;";
        ...
        sendBtn.textContent = alreadySent ? "Already Sent" : "Send to Developer";
        sendBtn.disabled = alreadySent;
        sendBtn.style.cssText = "flex:1;background:" + (alreadySent ? "#222" : "#248046") + ";color:" + (alreadySent ? "#555" : "#fff") + ";border:none;border-radius:10px;padding:12px;font-size:13px;font-weight:600;cursor:" + (alreadySent ? "not-allowed" : "pointer") + ";pointer-events:all;";
        sendBtn.onclick = async () => {
            if (localStorage.getItem(SENT_KEY) === "true") return;
            ...
            try {
                ...
                setTimeout(() => { sendBtn.textContent = "Send to Developer"; sendBtn.disabled = false; }, 2000);
            }
        };
